                                    ...
                                    int version, Subscription* subObj)
{
	...
	if (sub == SubscriptionType::READ)
		subscribeRead(dataId, subObj);
	else
		subscribeWrite(dataId);
}
...
{
	if (sub == SubscriptionType::READ)
		return doReadSubscription(id);
	else
		return doWriteSubscription(id);
}
...
{
	if (sub == SubscriptionType::READ)
		endDoReadSubscription(id);
	else
		endDoWriteSubscription(id);
}
